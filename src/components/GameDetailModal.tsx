...
import { X, Sparkles, Check, Calendar } from 'lucide-react';
...
export const GameDetailModal: React.FC<GameDetailModalProps> = ({
  ...
  const displayGenre = isPrizeDenied ? t.prizeDenied.genre : game.genre;
  const displayFullDesc = isPrizeDenied ? t.prizeDenied.fullDesc : game.fullDesc;
  const displayFeatures = isPrizeDenied ? t.prizeDenied.features : game.features;
  const displayReleaseDate = isPrizeDenied ? t.prizeDenied.releaseDate : game.releaseDate;
  const displayDeveloper = isPrizeDenied ? t.prizeDenied.developer : game.developer;
  const displayPublisher = isPrizeDenied ? t.prizeDenied.publisher : game.publisher;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-20 p-2 rounded-full bg-slate-950/80 hover:bg-slate-900 text-slate-400 hover:text-white border border-sky-800/40 transition-colors cursor-pointer"
        ...
        <div className="p-6 sm:p-8 space-y-6">
          ...
          <div className="space-y-3 text-sm text-slate-300 leading-relaxed border-t border-sky-900/40 pt-4">
            ...
            <div className="whitespace-pre-line text-slate-300 leading-relaxed space-y-2">{displayFullDesc}</div>
          </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-[#081126] p-4 rounded-xl border border-sky-950 text-xs">
            <div>
              <span className="text-slate-400 block mb-0.5">{t.modal.releaseStatus}</span>
            ...
            <div>
              <span className="text-slate-400 block mb-0.5">{t.modal.developer}</span>
              <span className="font-bold text-white">{displayDeveloper}</span>
            </div>
            <div>
              <span className="text-slate-400 block mb-0.5">{t.modal.publisher}</span>
              <span className="font-bold text-white">{displayPublisher}</span>
            </div>
            <div>
              <span className="text-slate-400 block mb-0.5">{t.modal.platform}</span>
              <a
                href="https://store.steampowered.com/app/3424940/_/"
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1.5 mt-1 text-xs font-bold text-sky-300 hover:text-white transition-colors cursor-pointer"
              >
                <SteamIcon size={18} />
                <span>Steam</span>
              </a>
